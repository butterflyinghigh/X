...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Masukkan Nomor : +62")))

            phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Ketik Nomor Anda 😍\nFor example: +62 : `)))
            phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
            rl.close()
         }
      }
...
try{
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	        ...
	        console.log(color(`\n${themeemoji} Lufi Chan`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: butterflyinghigh`,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @cttn.dakwah.hijrah `,'magenta'))
...
XeonLft = await getBuffer(ppuser)
                ...
                xeonbody = `┌─❖
│「 Ahlan Wa Sahlan Selamat Bergabung Semoga Betah. JANGAN LUPA BACA DESKRIPSI YA 👋 」
└┬❖ 「  @${xeonName.split("@")[0]}  」
   │✑  𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 
   │✑  ${metadata.subject}
   │✑  𝗠𝗲𝗺𝗯𝗲𝗿 : 
...
XeonBotInc.relayMessage(anu.id, msgs.message, {})
                    ...
                    xeonbody = `┌─❖
│「 Selamat Jalan . Semoga Tidak tersesat dari Jalan Allah Aamiin 👋 」
└┬❖ 「 @${xeonName.split("@")[0]}  」
   │✑  𝗟𝗲𝗳𝘁 
   │✑ ${metadata.subject}
   │✑  𝗠𝗲𝗺𝗯𝗲𝗿 : 
